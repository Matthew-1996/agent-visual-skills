...
2. 阅读 `${AGENT_VISUAL_HOME:-$HOME/agent-visual-skills}/codex/skills/infographic/references/layouts.md`，各选一个信息结构和克制风格。结构决定阅读顺序，风格只决定视觉语气。
...
6. 用 `"${AGENT_VISUAL_HOME:-$HOME/agent-visual-skills}/tools/bin/render-diagram" html` 分别渲染 1440×1100 与 390×844；检查浏览器错误、横向溢出、遮挡、截断、对齐和对比度，发现问题后修复并重渲染。
