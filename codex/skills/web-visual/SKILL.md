...
1. 阅读 `${AGENT_VISUAL_HOME:-$HOME/agent-visual-skills}/codex/skills/web-visual/references/patterns.md`、`${AGENT_VISUAL_HOME:-$HOME/agent-visual-skills}/shared/style-profiles/editorial-v1.1.md`、`${AGENT_VISUAL_HOME:-$HOME/agent-visual-skills}/shared/output-contract.md` 和 `${AGENT_VISUAL_HOME:-$HOME/agent-visual-skills}/shared/iconography.md`，选择一个主阅读模式；先写结论和决策问题，再安排证据。仅在用户明确要求时改用 `legacy-dark`。
...
6. 以 1440×1100 运行 `"${AGENT_VISUAL_HOME:-$HOME/agent-visual-skills}/tools/bin/render-diagram" html`；仅在用户明确请求移动端时增加 390×844。再实际触发关键交互，并按 `${AGENT_VISUAL_HOME:-$HOME/agent-visual-skills}/shared/visual-acceptance.md` 验收。
